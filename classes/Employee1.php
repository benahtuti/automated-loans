...
include_once ($filepath."/../libs/CrudOperation1.php");
include_once ($filepath."/../libs/Session1.php");
include_once ($filepath."/../helpers/Format1.php");
/**
...
class Employee
{
	...
	public function employeeLogin($data){
	    ...
	    if (empty($identity) or empty($password))
		{
			...
			$sql = "SELECT * FROM tbl_borrower WHERE nid='$nid' AND password='$password'";
	...
	public function viewBorrower()
	{
		...
		$sql = "SELECT * FROM borrower  ORDER BY identity DESC";
		$result = $this->db->select($sql);
		return $result;
	...
	public function findBorrower($nid)
	{
		...
		$sql = "SELECT * FROM borrower WHERE nid='$identity'";
		$result = $this->db->select($sql);
		return $result;
	}
	...
	{
		//get all borrower data by nid
		$sql = "SELECT * FROM borrower WHERE identity='$identity' ";
		$result = $this->db->select($sql);
		return $result;
	}
